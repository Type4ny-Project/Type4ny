			...
			<div class="_gaps_m">
				<SearchMarker :keywords="['open', 'registration']">
					<MkSwitch v-model="enableRegistration" :disabled="isManaged">
						<template #label><SearchLabel>{{ i18n.ts.enableRegistration }}
						</SearchLabel></template>
						<template #caption><p v-if="isManaged">{{ i18n.ts.managedInstanceIsNotEditable }}</p>
...
const enableGDPRMode = ref<boolean>(meta.enableGDPRMode);
const enableLoginBonus = ref<boolean>(meta.enableLoginBonus);
